...
<VOTABLE version="1.2" xmlns="http://www.ivoa.net/xml/VOTable/v1.2">
    ...
    <RESOURCE type="results">        
        ...
        <#if infos?exists>
          <#list infos as info>
            <INFO<#if info.id?exists> ID="${info.id}"</#if> name="${info.name}" value="${info.valueAttribute}"<#if info.xtype?exists> xtype="${info.xtype}"</#if><#if info.ref?exists> ref="${info.ref}"</#if><#if info.unit?exists> unit="${info.unit}"</#if><#if info.ucd?exists> ucd="${info.ucd}"</#if><#if info.utype?exists> utype="${info.utype}"</#if> ><#if info.value?exists>${info.value}</#if></INFO>
          </#list>
        </#if>
        <#if params?exists>
